...
$query = mysql_query("SELECT * FROM Corpo_Docente WHERE username='".$_POST["username"]."' AND password ='".$_POST["password"]."'")  //per selezionare nel db l'utente e pw che abbiamo appena scritto nel log
...
if(mysql_num_rows($query)!=0){        //se c'è una persona con quel nome nel db allora loggati
	...
	$_SEESION["idimpronta"]=$row["ImprontaID"];
        if($_POST["username"]=="capo"){
		header("location:logamm.php");
	}
	else{
		header("location:logprof.php"); // e mando per esempio ad una pagina esempio.php// in questo caso rimanderò ad una pagina prova.php
	}
}
else{
